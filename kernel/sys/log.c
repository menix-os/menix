...
int64_t str_to_int(const char* str, int base) {
...
uint64_t str_to_uint(const char* str, int base) {
    size_t len = strlen(str);
    uint64_t result = 0;
    size_t i = 0;
    for (; i < len; i++)
...
char* int_to_str(int64_t value, char* str, int base) {
    size_t i;
    int64_t sign;
    sign = value;
    if (base == 10 && sign < 0)
...
char* uint_to_str(uint64_t value, char* str, int base) {
    size_t i = 0;
    do {
        char c = value % base;
        if (c > 9)
...
check_fmt:
        ...
        case '.': {
            ...
            if (*fmt == '*') {
                ...
                precision = str_to_uint(number, 10);
            }
            goto check_fmt;
        }
        // Length
        ...
        case 'd': {
            switch (size) {
            case sizeof(int8_t):
                int_to_str((int8_t)va_arg(args, int32_t), number, 10);
                break;
            case sizeof(int16_t):
                int_to_str((int16_t)va_arg(args, int32_t), number, 10);
                break;
            case sizeof(int32_t):
                int_to_str((int32_t)va_arg(args, int32_t), number, 10);
                break;
            default:
                int_to_str(va_arg(args, int64_t), number, 10);
                break;
            }
            goto print_num;
        }
        ...
        case 'u': {
            switch (size) {
            case sizeof(uint8_t):
                uint_to_str((uint8_t)va_arg(args, uint32_t), number, 10);
                break;
            case sizeof(uint16_t):
                uint_to_str((uint16_t)va_arg(args, uint32_t), number, 10);
                break;
            case sizeof(uint32_t):
                uint_to_str(va_arg(args, uint32_t), number, 10);
                break;
            case sizeof(uint64_t):
                uint_to_str(va_arg(args, uint64_t), number, 10);
                break;
            }
            goto print_num;
        }
        ...
        case 'x': {
            switch (size) {
            case sizeof(uint8_t):
                uint_to_str((uint8_t)va_arg(args, uint32_t), number, 16);
                break;
            case sizeof(uint16_t):
                uint_to_str((uint16_t)va_arg(args, uint32_t), number, 16);
                break;
            case sizeof(uint32_t):
                uint_to_str(va_arg(args, uint32_t), number, 16);
                break;
            case sizeof(uint64_t):
                uint_to_str(va_arg(args, uint64_t), number, 16);
                break;
            }
print_num:
