...
pub fn register(device: Arc<dyn BlockDevice>, name: &str) -> EResult<()> {
    ...
    // Scan for partitions.
    let partitions = partition::GptPartition::scan(device.clone())?;

    for (i, part) in partitions.into_iter().enumerate() {
        let path = format!("/dev/{}p{}", name, i + 1);
        log!("GPT partition: {}, {}", path, part.name());
