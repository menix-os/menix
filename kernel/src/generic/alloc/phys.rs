...
pub fn alloc(num_pages: usize) -> Option<PhysAddr> {
    ...
    let mem = allocate_free_pages(&mut allocator, num_pages, last)?;
...
pub fn alloc_zeroed(num_pages: usize) -> Option<PhysAddr> {
    let addr = alloc(num_pages)?;
    unsafe {
        write_bytes(direct_access(addr), 0, 1 << PageTableEntry::get_page_bits());
    };
    return Some(addr);
}

pub fn alloc_bytes(bytes: usize) -> Option<PhysAddr> {
    let addr = alloc((bytes / (1 << PageTableEntry::get_page_bits())) + 1)?;
    unsafe {
        write_bytes(direct_access(addr), 0, 1 << PageTableEntry::get_page_bits());
    };
    return Some(addr);
...
fn allocate_free_pages(pmm: &mut PhysManager, amount: usize, start: usize) -> Option<PhysAddr> {
    for i in start..pmm.num_pages {
        // Loop until we find a free page.
        if pmm.bit_map.get(i).unwrap() {
            continue;
        }

        // After finding a single free page, check if there are more free pages.
        for k in i..(i + amount) {
            if pmm.bit_map.get(k).unwrap() {
                continue;
            }
        }
