...
void idt_reload()
{
	...
	isr_register_handler(cpu, 0x6, interrupt_ud_handler);
	isr_register_handler(cpu, 0xE, interrupt_pf_handler);
	isr_register_handler(cpu, INT_TIMER, timer_handler);
	isr_register_handler(cpu, INT_SYSCALL, syscall_handler);
